<template>
  <div>
    ...
    <div
      class="block max-w-sm p-4 bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700 mt-2"
    >
      <div
              ...
              <div class="fixed inset-0 overflow-y-auto">
                ...
                >
                  ...
                  >
                    ...
                    >
                      <TagsAdd @form-save="addTag" @hide="closeModal" />
                    </DialogPanel>
                  </TransitionChild>
                </div>
              </div>
...
const addTag = async (tagName: any) => {
  ...
  tags.value.unshift(data.value);
};
const emitData = (tag: Object) => {
  tag.value == "edit" ? (editInput.value = tag.tag) : deleteNote(tag);
  console.log("editIn23put", editInput);
};
// Delete tags
const deleteNote = (tag: any) => {
  useAuthLazyFetchDelete(`${props.url}${tag.tag.uid}`, {});
...
// Edit notes
const edit = (tag: any) => {
  useAuthLazyFetchPut(`${props.url}${tag.uid}?name=${tag.tag}`, {
    body: {
      project_id: props.project_id,
      name: tag.tag,
      entity: props.entity,
  ...
  tags.value.forEach((data: any) => {
    if (tag.uid === data.uid) {
      console.log("dattaaa", data, tag);
